...
public class PassiveAlgoLogic implements AlgoLogic {
    ...
    public Action evaluate(SimpleAlgoState state) {
        ...
        // neartouch at index 0...? Best Bid
        // highest price buyers currently willing to buy the asset
        final BidLevel nearTouch = state.getBidAt(0);

        long quantity = 75;

        // this is where the "passiveness" is from
        // algo wants to join passive side of order book by placing order to but at the
        ...
        if (state.getChildOrders().size() < 3) {
            ...
            logger.info("[PASSIVEALGO] Have:" + state.getChildOrders().size()
